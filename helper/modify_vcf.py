...
def prepareFile(filename):
	...
	Manta candidateSV and Manta somaticSV files have different formats. That needs to be addresed here, though the pipeline uses somaticSV
	"""
	data = []
	bnd_dict = {}
	has_DV = False
...
def main(args):
	...
	# Check if these files have been prepped previously,
	# which would be the case if these are rescued SVs 
	# from the second portion of the pipeline
	previously_prepped = False
	with open(input_files[0], "r") as f:
		for line in f:
			if "##Rescued" in line:
				previously_prepped = True
	f.close()
	# If previously_prepped, no changes need to be made. Just copy the original file.
	if previously_prepped:
		for i in range(0,len(input_files)):
			infile_root = input_files[i].split("/")[-1]
			infile_base = ".".join(infile_root.split(".")[:-1])
			outfile_name = out_dir + "/" + infile_base + ".vcf"
			with open(outfile_name, "w") as f:
				with open(input_files[i], "r") as infile:
					for line in infile:
						f.write(line)	
				infile.close()
			f.close()
